...
namespace TeachMe.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<IActionResult> Index(IndexViewModel model)
        {
            ...
            if (model.LastName != user.LastName || model.FirstName != user.FirstName || model.City != user.City || model.Biography != user.Biography || model.Skype != user.Skype)
            {
        ...
        public async Task<IActionResult> NotificateTeacher(string email)
        {
            EmailSender emailSender = new EmailSender();
            
            await emailSender.SendEmailAsync(email, "Warning!!", "U have lesson now");
            return RedirectToAction(nameof(Index));
        }

        public async Task<IActionResult> UploadCertificat(IndexViewModel vm) 
        {
            var fileName = Path.Combine($"{_appEnvironment.WebRootPath}\\images\\certificates\\", Path.GetFileName(vm.Image.FileName));
